...
public class MenuService {
	...
	public Menu findAllItems(String item) {
		try {
			return menurepo.findByItem(item);
		} catch (ItemNotFoundException exception) {
			throw new ItemNotFoundException("Food Items Doesn't Exist" + exception);
		}
	}
	...
	public Menu findCart(Cart cart) throws Exception {
		Menu menu = new Menu();
		...
		try {
			if (cart.quantity >= 1) {
				...
				throw new ArithmeticException("special character not allowed");
			}
